...
- **Learnings system design — Option C (2026-03-01)**: Constitutional "core learnings" file (curated, commonly read on fresh onboard, mix of types) + separate typed catalog files (comprehensive). Core entries duplicated in their proper type files — core is a self-contained read, not a pointer file. Five learning types: failure_pattern, process_rule, empirical_finding, architectural_rationale, domain_knowledge. Single `learnings-catalog.md` sectioned by type until ~500 entries, then split. Entry format defined (graph-ready markdown with ID, type, tags, confidence, related field with edge types). Design principles and relationship schema extracted to `DESIGN-PRINCIPLES.md`. All pruned knowledge from ONBOARDING.md active items must be preserved in the learnings system, not just git history. *Schema design complete — implementation not started.*
- ~~**ONBOARDING.md maintenance rules** — ✅ Completed 2026-03-01~~ (commits a8d0674, d8ad7c2). Rules live in "Keeping This File Current" section. *Remove this line on next fresh onboard.*
