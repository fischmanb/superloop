...
- **Recently completed (2026-03-04, evening)**: Test suite reliability overhaul. Orphaned `test_comp_detail.py` removed (broke pytest collection). `test_overnight_autonomous` hang fixed — 6 tests missing `generate_codebase_summary` mock spawned real `claude` CLI processes. `test_build_loop` 162s→0.1s — same root cause, 2 tests. L-00178 enforcement across 5 surfaces (tests, runtime warning, CLAUDE.md, core.md, process-rules.md). Full suite: **740 tests, 15.86s, zero hangs**. Main at `85ede30`.
