...
## Purpose
HOW-I-WORK-WITH-GENERATIVE-AI.md Accumulation section is a pre-learnings staging area.
Raw observations accumulate there (checkpoint step 5) before being synthesized into
actionable rules. This command closes the feedback loop (L-0124) by surfacing patterns
that should become learnings or protocol changes.
...
- If already captured → mark signal as "covered by L-XXXX"
...
Already covered:
  - "agent trust assumptions" cluster (3 entries) → covered by L-0001, L-0005
  - "context window pressure" (2 entries) → covered by L-0130
...
Refinement candidates:
  - "fresh context as stress test" (2 entries) → L-0117 covers adoption latency
...
- Covered entries: optionally mark in Accumulation with `[→ L-XXXX]` so future
...
## When to Run
- During checkpoint step 4 methodology signals review (L-0124)
